...
| 工具 | 说明 | 入口 | 成本 |
|---|---|---|---|
| 🧽 **图片去水印** | 框选区域智能去除水印/Logo/文字 | Web 工作台 → 图片工具 | 0 元（本地处理） |

**图片去水印支持3种算法**：

| 算法 | 类型 | 效果 | 费用 | 说明 |
|---|---|---|---|---|
| OpenCV TELEA | 传统 | ⭐⭐ | 免费 | 速度最快，适合小面积纯色背景水印，无需额外配置 |
| OpenCV NS | 传统 | ⭐⭐ | 免费 | 边缘衔接更自然，速度稍慢，无需额外配置 |
| AI 火山引擎（云端） | AI云端 | ⭐⭐⭐⭐⭐ | 按量计费 | 商用级擦除效果最佳，需配置 `VOLC_ACCESS_KEY`/`VOLC_SECRET_KEY` |
...
- 📋 6 种视频工作流卡片选择（含卡片轮播视频）
- 🛠️ 图片工具区：图片去水印（框选区域，智能去除）
...
- zentrix566 - 项目创始人、核心开发
- [ruixuan-xi](https://github.com/ruixuan-xi) - 项目合作人、贡献者
